...
public abstract class EntitySupersonicCruiseMissileBase extends Entity implements IChunkLoader, IRadarDetectable {
	...
	public EntitySupersonicCruiseMissileBase(World p_i1582_1_) {
		super(p_i1582_1_);
	...
	public EntitySupersonicCruiseMissileBase(World world, float x, float y, float z, int a, int b, int c) {
		super(world);
		this.ignoreFrustumCheck = true;
		/*this.posX = x;
		this.posY = y;
    ...
    public void onUpdate() {
		...
		//1.Position
        CruiseMissilePosition = Math.sqrt(((this.posX - startX)*(this.posX - startX)) + 0 + ((this.posZ - startZ)*(this.posZ - startZ)));
		...
		if(this.ticksExisted > 40)
			...
			velocity = 4;
		if(this.CruiseMissilePosition > this.startsonicspeed && isSupersonic && !this.worldObj.isRemote)
	    	velocity = 5;
		...
		for(int i = 0; i < velocity; i++) {
	        ...
	        //3. Bedingungen f�r Transformation
